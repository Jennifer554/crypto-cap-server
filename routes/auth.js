...
(async () => {
  ...
  auth.post("/sendForgotPasswordEmail/", async (req, res) => {
    ...
    user.save();
    var mailOptions = {
      from: '"Crypti Cap" <crypticap9@gmail.com>', // sender address
      to: `${email}`, // list of receivers
      subject: "CryptiCap || Reset Your Password ✔", // Subject line
  ...
  auth.post("/sendVerifyEmail/", async (req, res) => {
    ...
    var mailOptions = {
      from: '"Crypti Cap" <crypticap9@gmail.com>', // sender address
      to: `${email}`, // list of receivers
      subject: "CryptiCap || Verify your Email Address ✔", // Subject line
      html: `<div style="padding: 0 30px ;"><h4>Hi There!</h4> <p>
      Welcome to Crypti Cap! Ready to experience all the benefits of being financially independent? Before you head off to the races, make sure to verify your email address using the code below:
